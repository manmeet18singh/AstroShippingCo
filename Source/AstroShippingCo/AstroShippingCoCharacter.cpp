...
AAstroShippingCoCharacter::AAstroShippingCoCharacter()
{
	...
	// Load Use Montage
	static ConstructorHelpers::FObjectFinder<UAnimMontage> UnequipMontageObject(TEXT("AnimMontage'/Game/Animations/Zazi/HeatGun/AM_HeatGun_Unequip.AM_HeatGun_Unequip'"));
	if (UnequipMontageObject.Succeeded())
	{
		UnequipMontage = UnequipMontageObject.Object;
	}

	static ConstructorHelpers::FObjectFinder<UAnimMontage> FireMontageObject(TEXT("AnimMontage'/Game/Animations/Zazi/HeatGun/AM_HeatGun_Fire.AM_HeatGun_Fire'"));
	if (FireMontageObject.Succeeded())
	{
		FireMontage = FireMontageObject.Object;
	}

	static ConstructorHelpers::FObjectFinder<UAnimMontage> InteractMontageObject(TEXT("AnimMontage'/Game/Animations/Zazi/AM_Zazi_Use.AM_Zazi_Use'"));
...
void AAstroShippingCoCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Setup for fire
	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &AAstroShippingCoCharacter::Fire);
...
void AAstroShippingCoCharacter::UseItemAtInventorySlot(int32 Slot)
{
	if (Inventory[Slot] != NULL)
	{
		...
		if (Inventory[Slot]->IsEquippable() && !Inventory[Slot]->IsEquipped() && !characterEquipStatus)
		{
			CurrentEquip = Inventory[Slot];

			Inventory[Slot]->Equip_Implementation();
			characterEquipStatus = true;
		...
		else if (Inventory[Slot]->IsEquippable() && !Inventory[Slot]->IsEquipped() && characterEquipStatus)
		{
			...
			//UseItemAtInventorySlot(Slot);
		}
		// Current selected ability is equipped and ability is also equipped
		else if (Inventory[Slot]->IsEquippable() && Inventory[Slot]->IsEquipped() && characterEquipStatus)
		{
			UnequipAtInventorySlot(Slot);
		}
...
void AAstroShippingCoCharacter::Fire()
{
	if (CurrentEquip != NULL)
	{
		CurrentEquip->Use_Implementation();
		PlayAnimMontage(UnequipMontage, 1.0f, FName("Default"));
	}
}
