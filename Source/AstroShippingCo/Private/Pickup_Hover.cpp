...
void APickup_Hover::Equip_Implementation()
{
	...
	if (ToSpawn)
	{
		...
		if (world)
		{
			...
			const FTransform socketTransform = Character->GetMesh()->GetSocketTransform(FName("JetpackSocket"), ERelativeTransformSpace::RTS_World);

			AActor* spawnedAbility = world->SpawnActor(ToSpawn, &socketTransform);

			if (spawnedAbility == nullptr) return;

			spawnedAbility->AttachToComponent(Character->GetMesh(), FAttachmentTransformRules::SnapToTargetNotIncludingScale, FName("JetpackSocket"));
		}
